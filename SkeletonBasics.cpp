...
int anglepass(UINT t1, UINT t2, UINT t3, UINT t4)
{
	BYTE iDegree= 0, bRet;
	//iDegree = t1 ;
    ...
    m_pNuiSensor(NULL)
{
	...
	ZeroMemory(m_PrevPoints, sizeof(m_Points));
	m_uiDrawCnt = 0;
}
...
HRESULT CSkeletonBasics::CreateFirstConnected()
{
   ...
   HRESULT hr = NuiGetSensorCount(&iSensorCount);
	...
	SP = new Serial("COM3");    // adjust as needed

	if (SP->IsConnected())
		OutputDebugStringA("We're connected\n");
	else
	{
		OutputDebugStringA("Serial not connected\n");
		return E_FAIL;
	}
...
{      
	int i;
	WCHAR awAngles[256] = { 0 };

    for (i = 0; i < NUI_SKELETON_POSITION_COUNT; ++i)
    {
		WCHAR awcCo[256] = { 0 };
		// static const WCHAR awcCo[] = L"Hello, World!";

        m_Points[i] = SkeletonToScreen(skel.SkeletonPositions[i], windowWidth, windowHeight);

		//if (0 == m_uiDrawCnt % 10)
		//wsprintfW(awcCo, L"[%d] x %ld y %ld", i, m_Points[i].x, m_Points[i].y);
		//m_pRenderTarget->DrawTextW(awcCo, wcslen(awcCo), m_pTextFormat,
			//D2D1::RectF(400, 100+i*20, windowWidth, windowHeight), m_pBrushJointInferred);
		if (m_PrevPoints[i].x != 0 && m_PrevPoints[i].y != 0)
		{
			
			//angle=asin(sqrt(pow( m_Points[i].x - m_PrevPoints[i].x,2.0)+pow(m_Points[i].y - m_PrevPoints[i].y,2.0))/sqrt(pow(m_Points[i].x,2.0)+pow(m_Points[i].y,2.0)));
			//wsprintfW(awcCo, L"[%d] x %ld y %ld angle %ld\n", i, m_Points[i].x - m_PrevPoints[i].x, m_Points[i].y - m_PrevPoints[i].y,angle);
			//m_pRenderTarget->DrawTextW(awcCo, wcslen(awcCo), m_pTextFormat,
				//D2D1::RectF(600, 100 + i * 20, windowWidth, windowHeight), m_pBrushJointInferred);
		}
    }
	if (0 == m_uiDrawCnt % 2)
	{
		Vector4 vRS = skel.SkeletonPositions[NUI_SKELETON_POSITION_SHOULDER_RIGHT];
		Vector4 vRE = skel.SkeletonPositions[NUI_SKELETON_POSITION_ELBOW_RIGHT];
		Vector4 vRW = skel.SkeletonPositions[NUI_SKELETON_POSITION_WRIST_RIGHT];
		swprintf(awAngles, L"[%d] rsx %f rex %f rsy %f rey %f rsz %f rez %f \n", m_uiDrawCnt, vRS.x, vRE.x, vRS.y, vRE.y, vRS.z, vRE.z);
		//m_pRenderTarget->DrawTextW(awAngles, wcslen(awAngles), m_pTextFormat,
		//	D2D1::RectF(400, 100 + 100, windowWidth, windowHeight), m_pBrushJointInferred);
		OutputDebugStringW(awAngles);

		float ABx = vRS.x - vRE.x;//mod(vRS.x, vRE.x);
		float ABy = vRS.y - vRE.y;//mod(vRS.y, vRE.y);
		float ABz = vRS.z - vRE.z;//mod(vRS.z, vRE.z);
		float denom1 = sqrt(pow(ABx, 2) + pow(ABy, 2) + pow(ABz, 2));
		float BCx = vRE.x - vRW.x;//mod(vRE.x, vRW.x);
		float BCy = vRE.y - vRW.y;//mod(vRE.y, vRW.y);
		float BCz = vRE.z - vRW.z;//mod(vRE.z, vRW.z);
		float ACx = vRS.x - vRW.x;
		float ACy = vRS.y - vRW.y;
		float denom2 = sqrt(pow(BCx, 2) + pow(BCy, 2) + pow(BCz, 2));
		swprintf(awAngles, L"~~~~~~~~~~~[%d] abx %f aby %f abz %f denom %f \n", m_uiDrawCnt, ABx, ABy, ABz, denom1);
		//m_pRenderTarget->DrawTextW(awAngles, wcslen(awAngles), m_pTextFormat,
		//	D2D1::RectF(400, 100 + 100, windowWidth, windowHeight), m_pBrushJointInferred);
		OutputDebugStringW(awAngles);
		...
		WCHAR wcTemp[128];
		float theta1 = asin(-ABx / denom1);
		float theta2 = asin(-ABy / denom1);
		float theta3 = acos(BCx / denom2);
		float theta4 = acos(BCy / denom2);
		//float theta4 = acos(pow(AC, 2) - pow(AB, 2) - pow(BC, 2));
		swprintf(wcTemp, L"Rad %f %f %f %f\n", theta1, theta2, theta3, theta4);
		OutputDebugStringW(wcTemp);
		
		INT t1 = RAD2DEG(theta1);
		INT t2 = RAD2DEG(theta2);
		INT t3 = RAD2DEG(theta3);
		INT t4 = RAD2DEG(theta4);
		
		
		swprintf(wcTemp, L"ORig %d %d %d %d\n", t1, t2, t3, t4);
		OutputDebugStringW(wcTemp);
		t2 = t2 + 90;
		if (vRS.z - vRE.z > 0)
		{
			OutputDebugStringA("+ve");
		}
		else {
			OutputDebugStringA("-ve");
			t1 = 180 - t1;
		...
		//t3 = 180 + t3;
		//swprintf(wcTemp, L"Deg -ve %d %d %d %d\n", t1, t2, t3, t4);
		//OutputDebugStringW(wcTemp);
		swprintf(wcTemp, L"Deg[%d] %d %d %d %d\n", m_uiDrawCnt, t1, t2, t3, t4);
		//OutputDebugStringA("hello0\n");
		OutputDebugStringW(wcTemp);

		if (SP->IsConnected())
		{
			INT iRet = anglepass(t1, t2, t3, t4);
			//anglepass(t1);
			swprintf(wcTemp, L"Deg[%d] %d\n", m_uiDrawCnt, iRet);
			OutputDebugStringW(wcTemp);
		}
		//m_pRenderTarget->DrawTextW(wcTemp, wcslen(wcTemp), m_pTextFormat,
		//	D2D1::RectF(400, 100 + 200, windowWidth, windowHeight), m_pBrushJointInferred);
